...
function tableBlockHtml(t, si, ti) {
    ...
    const headerHtml = cols.length > 0
        ...
        : "";
    const bodyHtml = (t.rows ?? []).map((row) => `<tr>${row.map((cell) => `<td><input type="text" class="te-cell" value="${escAttr(cell)}"></td>`).join("")}<td class="te-action-col"><button type="button" class="btn-remove-row" title="Remove row">✕</button></td></tr>`).join("");
    return `
