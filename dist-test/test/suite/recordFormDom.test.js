...
function loadForm(fieldValues, fields) {
    const body = (0, forms_1.buildRecordForm)({ instanceId: "rec-1", typeId: "type-1", typeName: "t", typeNamespace: "com.example", typeVersion: 1, fieldValues }, fields);
    const dom = new jsdom_1.JSDOM(`<!doctype html><html><body><form id="editor-form">${body}</form></body></html>`, {
...
describe("record form — client-side DOM interaction", () => {
    it("adding a value to the SECOND cloned list-composite entry's nested repeatable field lands in that entry, not the first", () => {
        const window = loadForm({ rows: [{ cells: ["existing"] }] }, [
            {
                name: "rows",
                displayLabel: "Rows",
                order: 0,
        ...
        const addRowBtn = document.querySelector('[data-field="rows"] > .btn-add-entry[data-add="entry"]');
        ...
        const addCellBtn = secondEntry.querySelector('[data-field="cells"] .btn-add-entry[data-add="value"]');
    ...
    it("collects an unset optional enum field as absent, not as the first enum option", () => {
        const window = loadForm({ title: "Board" }, [
        ...
        const collected = window.collectFormData();
        assert.strictEqual("theme" in collected.fieldValues, false, "an unset optional enum field must be omitted (RFC-039 absence=unset), not defaulted to the first option");
    });
    it("preserves a stored enum value that is no longer in the current vocabulary, instead of silently dropping it", () => {
        const window = loadForm({ theme: "legacy-value" }, [{ name: "theme", displayLabel: "Theme", order: 0, required: false, kind: "enum", enumValues: ["default", "inverted"] }]);
        const select = window.document.querySelector('[data-field="theme"] select');
    ...
    it("omits an optional composite field left entirely blank, instead of sending {}", () => {
        ...
        const collected = window.collectFormData();
        assert.strictEqual("location" in collected.fieldValues, false, "a composite field with no children filled in must be omitted (absence=unset), not sent as {}");
    });
    it("coerces a boolean scalar field even with trailing whitespace from the textarea", () => {
        const window = loadForm({ active: true }, [{ name: "active", displayLabel: "Active", order: 0, required: false, kind: "scalar", scalarType: "boolean" }]);
        const textarea = window.document.querySelector('[data-field="active"] textarea');
        textarea.value = "true\n";
        const collected = window.collectFormData();
    ...
    it("round-trips an unexpanded object-shaped field (map datatype / unresolved composite) through JSON, not as a stringified blob", () => {
        const window = loadForm({ config: { retries: 3, enabled: true } }, [{ name: "config", displayLabel: "Config", order: 0, required: false, kind: "scalar", scalarType: "json" }]);
        const collected = window.collectFormData();
        assert.deepStrictEqual(JSON.parse(JSON.stringify(collected.fieldValues.config)), { retries: 3, enabled: true }, "an untouched save must preserve the object, not corrupt it into a JSON-encoded string");
    });
    it("preserves a blank middle entry in a list-scalar field at its position, instead of dropping it and shifting later entries", () => {
        const window = loadForm({ cells: ["Yes", "", "See note"] }, [{ name: "cells", displayLabel: "Cells", order: 0, required: true, kind: "list-scalar" }]);
        const collected = window.collectFormData();
        assert.deepStrictEqual(JSON.parse(JSON.stringify(collected.fieldValues.cells)), ["Yes", "", "See note"], "a deliberately blank middle entry must survive an untouched save at its original position");
    });
    it("round-trips a list-of-map field (cardinality:list over datatype:map) through JSON per entry, not as stringified blobs", () => {
        const window = loadForm({ configs: [{ retries: 3 }, { retries: 5 }] }, [{ name: "configs", displayLabel: "Configs", order: 0, required: false, kind: "list-scalar", scalarType: "json" }]);
        const collected = window.collectFormData();
        assert.deepStrictEqual(JSON.parse(JSON.stringify(collected.fieldValues.configs)), [{ retries: 3 }, { retries: 5 }], "an untouched save must preserve each entry as an object, not corrupt it into a JSON string");
    });
    it("trims leading/trailing whitespace from a plain string scalar on save, matching the pre-RFC-039 editor", () => {
        const window = loadForm({ title: "Board" }, [{ name: "title", displayLabel: "Title", order: 0, required: true, kind: "scalar" }]);
        const textarea = window.document.querySelector('[data-field="title"] textarea');
    ...
    it("does not silently turn Infinity/-Infinity into a JSON null on save", () => {
        const window = loadForm({ count: 5 }, [{ name: "count", displayLabel: "Count", order: 0, required: false, kind: "scalar", scalarType: "number" }]);
        const textarea = window.document.querySelector('[data-field="count"] textarea');
