// Resolves a Type's effective field set for record editing/preview, via `type schema`
// (RFC-039/RFC-032): the CLI already expands composite ranges and orders/labels fields,
// so this is a thin parse of that JSON Schema rather than a client-side re-derivation.
import { CliClient } from "./CliClient";

export type FieldKind = "scalar" | "enum" | "list-scalar" | "composite" | "list-composite";

export interface ResolvedField {
  /** fieldValues carrier key — Field.name verbatim (RFC-039 R2b). */
  name: string;
  displayLabel: string;
  description?: string;
  order: number;
  required: boolean;
  kind: FieldKind;
  /** JSON Schema `type` for a scalar leaf (string/number/integer/boolean) — drives value coercion. */
  scalarType?: string;
  widget?: string;
  /** True when the scalar's `contentMediaType` is `text/markdown` — render as markdown, not plain text. */
  isMarkdown?: boolean;
  enumValues?: string[];
  minItems?: number;
  maxItems?: number;
  /** Present for "composite" and "list-composite" — the nested field set. */
  children?: ResolvedField[];
}

interface TypeSchemaPayload {
  schema: {
...
function looksLikeShortLabel(t: string): boolean {
  return t.length <= 50 && t.trim().split(/\s+/).length <= 6;
}

function parseField(name: string, node: JsonSchemaNode, required: boolean): ResolvedField {
  const base = {
    name,
...
export async function resolveTypeFields(
  cli: CliClient,
  repoPath: string,
  typeId: string,
  ...
  if (typeVersion !== undefined) args.push("--type-version", String(typeVersion));
  const payload = await cli.runOk<TypeSchemaPayload>(repoPath, args);
