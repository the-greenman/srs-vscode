...
// RFC-039 carrier: fieldValues is a JSON object keyed by Field.name, recursive over
// scalar / list / inline-composite shapes. Mirrors cli/typeFields.ts's ResolvedField —
// kept as a structural (not imported) type to preserve this module's no-vscode-deps rule.
...
export type TypeFieldKind = "scalar" | "enum" | "list-scalar" | "composite" | "list-composite";
...
function renderField(f: TypeFieldData, value: unknown): string {
  ...
  switch (f.kind) {
    case "enum": {
      ...
      const options = [`<option value=""${current === "" ? " selected" : ""}></option>`]
        ...
        .concat(
          current !== "" && !knownValues.includes(current)
    ...
    case "list-composite": {
      ...
      const renderEntry = (entryValue: Record<string, unknown>) => `
      ...
      const entriesHtml = items
        .map((it) => renderEntry((it && typeof it === "object") ? it as Record<string, unknown> : {}))
        .join("");
...
export function buildRecordForm(record: RecordData, fields: TypeFieldData[]): string {
