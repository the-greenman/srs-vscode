...
interface GraphNode {
  id: string;
  label: string;
  kind: string; // entity kind for openEntity
}
...
export class GraphPanel implements vscode.Disposable {
  private static readonly _panels = new Map<string, GraphPanel>();
  ...
  private async _load(cli: CliClient, repoPath: string): Promise<void> {
...
function graphHtml(nodes: GraphNode[], edges: GraphEdge[]): string {
