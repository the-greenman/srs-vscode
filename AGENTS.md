...
- **Gates**: payload-contract sync with `srs-rust` (see `CLAUDE.md`); the `schemas/2.0/` mirror is read-only — never edit it directly, sync from the spec repo per `CLAUDE.md`'s Schema Sync section.
